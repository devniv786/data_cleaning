...
def process_futures_data(df_futures: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        df_futures["Future_Series"] = df_futures["Future_Series"].replace({'I.NFO': "FUT-1", "II.NFO": "FUT-2",
                                                                            "III.NFO": "FUT-3"})
...
def calculate_options_parameters(ticker:str):
    if("TV18BRDCST" in ticker):
        ...
        expiry = ticker[idx_num1 + idx_num2.start()+1:idx_num1 + idx_num2.start()+1 + 7]
        strike_idx = re.search(r"\D", ticker[idx_num1 + idx_num2.start()+1 + 7:]).start()
        strike_price = ticker[idx_num1 + idx_num2.start()+1 + 7:][:strike_idx]
        option_type = ticker[idx_num1 + idx_num2.start()+1 + 7:][strike_idx:strike_idx + 2]
...
if __name__=="__main__":
    ...
    conn = psycopg2.connect( database="FNO", user='postgres', password='admin', host='127.0.0.1', port='5432')
    ...
    if(df_options_raw.shape[0]> 0):
        ...
        df_options_processed["Expiry"] = pd.to_datetime(df_options_processed["Expiry"],format="%d%b%y")
        df_options_processed["Week_Month"] = df_options_processed["Expiry"].dt.date.apply(lambda x : week_number_of_month(x))
        df_options_processed["Month_End"] = df_options_processed["Expiry"].apply(
                                                                                lambda x: pd.Period(x, freq="M").end_time.date())
        df_options_processed["Month_End"] = pd.to_datetime(df_options_processed["Month_End"],format="%Y-%m-%d")
        ...
        df_options_processed["Days_Diff"] = df_options_processed["Day_Month_End"] - df_options_processed["Day_Month"]
        df_options_processed["Expiry_Type"] = np.where(df_options_processed["Days_Diff"] <= 7, "Monthly", "Weekly")
        df_options_processed["Expiry"] = df_options_processed["Expiry"].astype(str)
        df_options_processed.drop(columns=["Month_End","Day_Month","Day_Month_End","Days_Diff","Week_Month"],inplace=True)
        df_options_processed["Future_Series"] = np.NaN
        df_options_processed = df_options_processed[['Ticker', 'Date', 'Time', 'DateTime', 'Open', 'High', 'Low', 'Close', 'Volume', 'Open Interest', 'Symbol', 'Future_Series', 'Expiry', 'Strike_Price', 'Option_Type', 'Expiry_Type']]
        df_futures_processed = df_futures_processed[['Ticker', 'Date', 'Time', 'DateTime', 'Open', 'High', 'Low', 'Close', 'Volume', 'Open Interest', 'Symbol', 'Future_Series', 'Expiry', 'Strike_Price', 'Option_Type', 'Expiry_Type']]
        df_final = pd.concat([df_futures_processed,df_options_processed],axis=0)
        df_final.rename(columns= {'Date':"Created_Date","Time":"Created_Time","DateTime":"Created_Date_Time",
